...
cd applications/challenges/app-manifests/
...
sed -i "s/<TAG>/${VERSION}/g" challenges-deployment.yaml
sed -i "s|<IMAGE_NAME>|${IMAGE_NAME}|g" challenges-deployment.yaml

kubectl apply -f ../datastore
kubectl apply -f .

echo "Describing the configurations"
kubectl describe deploy challenges
