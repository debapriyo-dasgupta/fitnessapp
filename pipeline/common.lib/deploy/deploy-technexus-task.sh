...
cd applications/technexus/env-manifests/$1
kubectl apply -f .
...
cd ../../manifests
sed -i "s/<TAG>/${VERSION}/g" technexus-deploy.yaml
sed -i "s|<IMAGE_NAME>|${IMAGE_NAME}|g" technexus-deploy.yaml

kubectl apply -f .

echo "Describing the configurations"
kubectl describe cm technexus-msr-appprop-cm webmethodslicensekeys
kubectl describe deploy technexus-msr